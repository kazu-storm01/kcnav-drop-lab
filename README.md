...
1. `outputs/kcnav-drop-lab.html` をブラウザで開く（サンプル海域入り）
2. 海域セレクトの「＋ 新しい海域を登録…」からKCNavのドロップ表を貼り付ける（自動解析）
3. 目標艦と勝利評価を選び、1周・10周・指定周回をシミュレーションする
...
- 目標・残り周回数・運勢を1文に集約したヒーロー表示
- 確率カードと周回数プリセット（「詳しい確率」に格納）
